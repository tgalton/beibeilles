...
@patch("app.services.baseline_detection_service.measurement_corrected_repository")
def test_detect_latest_baseline_not_enough_data(
    ...
    result = baseline_detection_service.detect_latest_baseline(
        db=Mock(),
        hive_level_id=1,
    )
...
# Fenêtre non stable
@patch("app.services.baseline_detection_service.measurement_corrected_repository")
@patch("app.services.baseline_detection_service.weight_baseline_service")
def test_detect_latest_baseline_unstable(
    ...
    mock_repository.get_latest_for_hive_level.return_value = [
        Mock(corrected_weight_kg=50) for _ in range(6)
    ]
...
def test_detect_latest_baseline_success(
    ...
    (mock_baseline_service.save_baseline.return_value) = baseline
